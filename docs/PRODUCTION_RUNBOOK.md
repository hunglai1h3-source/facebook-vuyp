...
Set `ALLOWED_HOSTS` to a comma-separated list after the final Render/custom domain is known. Create the first role-based admin by promoting an existing verified user in PostgreSQL:
...
- Provider-managed daily PostgreSQL backups/PITR are the primary recovery layer. Enable the longest retention allowed by the selected production plan.
...
The script reads `DATABASE_URL`; it never contains or prints the database password. Set `BACKUP_DIR` and `BACKUP_RETENTION_DAYS` as needed.
...
The restore script deliberately refuses to target `DATABASE_URL` and verifies the backup checksum when available.
...
- Operational logs expire after 30 days by default; admin audit logs expire after 365 days. Cleanup runs at most every six hours and deletes only the two log tables.
