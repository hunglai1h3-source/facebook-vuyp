...
def register_admin_ops(app, services):
        ...
        if services["postgres_enabled"]():
            ...
            with services["postgres_connect"]() as conn:
                with conn.cursor() as cur:
                    ...
                    legacy_total = sum(int(row.get("total", 0)) for row in legacy_rows)
            ...
            total_campaigns = sum(lifecycle.values()) + legacy_total
    ...
    def admin_users():
        ...
        page_items, pagination = _paginate(users, _int_arg("page"), 25)
    ...
    def admin_campaigns():
        ...
        page_items, pagination = _paginate(rows, _int_arg("page"), 40)
    ...
    def admin_campaign_detail(campaign_id):
        ...
        if not campaign:
            abort(404)
        tasks = [item for item in _all_tasks(users) if item.get("campaign_id") == campaign_id and item.get("customer_id") == campaign.get("customer_id")]
        campaign = _enrich_campaigns([campaign], tasks, users, _all_devices(users))[0]
        page_tasks, pagination = _paginate(tasks, _int_arg("page"), 50)
        return render_template("admin/campaign_detail.html", campaign=campaign, tasks=page_tasks, pagination=pagination)
